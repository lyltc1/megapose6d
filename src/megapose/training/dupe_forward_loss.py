...
from megapose.lib3d.rotations import random_rotation_matrix
...
def dupe_forward_loss(
    ...
    hypotheses_labels = np.repeat(
        np.expand_dims(np.array(labels, dtype=object), axis=1), cfg.n_hypotheses, axis=1
    ).copy()
    ...
    if cfg.hypotheses_init_method == "coarse_z_up+auto-depth":
        ...
        TCO_init_zup = TCO_init_from_boxes_zup_autodepth(bboxes, points_3d, K)
        ...
        TCO_init_random = add_noise(
            TCO.unsqueeze(1).repeat(1, n_hypotheses, 1, 1).flatten(0, 1),
    ...
    elif cfg.hypotheses_init_method == "SO3_gird_and_gt_noise":
        points_3d = mesh_db.select(np.ravel(hypotheses_labels).tolist()).sample_points(200)
        TCO_init_random = TCO_init_from_boxes_zup_autodepth(bboxes, points_3d, K)
        TCO_init_random = TCO_init_random.unsqueeze(1).repeat(1, n_hypotheses, 1, 1)
        TCO_init_random[..., :3, :3] = random_rotation_matrix(TCO_init_random.shape[:-2], device)
        TCO_init_gt_noise = add_noise(
            TCO.unsqueeze(1).repeat(1, n_hypotheses, 1, 1).flatten(0, 1),
            euler_deg_std=cfg.init_euler_deg_std,
            trans_std=cfg.init_trans_std,
        ).view(batch_size, cfg.n_hypotheses, 4, 4)

        probability = np.random.rand(batch_size, n_hypotheses)
        hypotheses_TCO_init = torch.where(
            torch.tensor(probability > 0.5, device=device, dtype=torch.bool),
            TCO_init_random,
            TCO_init_gt_noise,
        )
        is_hypothesis_positive = None
    ...
    outputs = model(
        ...
        box_modal=bboxes[hypotheses_image_ids_flat],
    ...
    TCO_possible_gt = TCO.unsqueeze(1) @ meshes.symmetries
