...
class CoarsePointMatching(nn.Module):
    ...
    def forward(self, p1, f1, geo1, p2, f2, geo2, radius, end_points):
        ...
        f1 = self.in_proj(f1)
        f1 = torch.cat([self.bg_token.repeat(B,1,1), f1], dim=1) # adding bg, [B, N+1, C]
        f2 = self.in_proj(f2)
        f2 = torch.cat([self.bg_token.repeat(B,1,1), f2], dim=1) # adding bg, [B, N+1, C]
        ...
        if self.training or 'model' not in end_points:
