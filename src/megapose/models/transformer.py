...
class RPEAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        """ args: input_states [B, N, dim], memory_states [B, N, dim], position_states [B, N, N, dim]
            return: output_states [B, N, dim], attention_scores [B, H, N, N] """
        hidden_states, attention_scores = self.attention(
            input_states,
            memory_states,
            memory_states,
...
class GeometricTransformer(nn.Module):
    ...
    def forward(self, feats0, embeddings0, feats1, embeddings1, masks0=None, masks1=None):
        """
        Args:
            feats0 [B, N, dim] embeddings0 [B, 1+N, 1+N, dim] feats1 [B, N, dim] embeddings1 [B, 1+N, 1+N, dim].
        """
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
