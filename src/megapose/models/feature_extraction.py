...
class ViTEncoder(nn.Module):
    ...
    def forward(self, end_points):
        ...
        dense_fm, img_class_token = self.get_img_feats(rgb, rgb_choose)
        dense_pm = end_points['pts']
        assert rgb_choose.size(1) == self.npoint

        if not self.training and 'dense_po' in end_points.keys() and 'dense_fo' in end_points.keys() and 'tem_class_token' in end_points.keys():
            ...
            tem_class_token = end_points['tem_class_token']
            ...
            (dense_po, dense_fo), tem_class_token= self.get_obj_feats(
                [tem1_rgb, tem2_rgb],
                [tem1_pts, tem2_pts],
                [tem1_choose, tem2_choose]
            )

        return dense_pm, dense_fm, dense_po, dense_fo, img_class_token, tem_class_token, radius

    def get_img_feats(self, img, choose):
        patch_feat, cls_token = self.rgb_net(img)
        return get_chosen_pixel_feats(patch_feat, choose), cls_token
    ...
    def get_obj_feats(self, tem_rgb_list, tem_pts_list, tem_choose_list, npoint=None):
        ...
        tem_feat_list =[]
        tem_class_token_list = []
        for tem, tem_choose in zip(tem_rgb_list, tem_choose_list):
            tem_feat, tem_class_token = self.get_img_feats(tem, tem_choose)
            tem_feat_list.append(tem_feat)
            tem_class_token_list.append(tem_class_token)

        tem_pts = torch.cat(tem_pts_list, dim=1)
        tem_feat = torch.cat(tem_feat_list, dim=1)
        tem_class_token = torch.cat(tem_class_token_list, dim=1)
        return sample_pts_feats(tem_pts, tem_feat, npoint), tem_class_token
